...
unsigned char gImage_qq_logo[3200] = 
{
...
void showimage(const unsigned char *p) //显示40*40 QQ图片
{
	...
	oled_fill(0 ,0 ,LCD_W ,LCD_H ,WHITE); //清屏  
	oled_delay_ms(800);
	...
	for(k=0;k<3;k++)
	{
	   	for(j=0;j<3;j++)
		{	
			oled_setwindows(40*j,40*k,40*j+39,40*k+39);		//坐标设置
			for(i=0;i<40*40;i++)
			{	
				picL=*(p+i*2);	//数据低位在前
				picH=*(p+i*2+1);				
				oled_write_data_16Bit(picH<<8|picL);					
			}
		 }
		oled_write_cmd(WRAMCMD);		        //开始写入GRAM
		oled_write_cmd(WRAMCMD);		        //开始写入GRAM
...
//采用设置窗口方法填充字库数据
void Fast_DrawFont_GBK16(uint8_t x, uint8_t y, uint8_t fc, uint8_t bc, const char *s)
{
	unsigned char i,j;
	unsigned short k;
	uint HZnum;
	HZnum=sizeof(stxt)/sizeof(typFNT_GB16);
	oled_fill(0 ,0 ,LCD_W ,LCD_H ,WHITE); //清屏 
	ESP_LOGI(TAG, "1111111111111111111111111111111111\n ");
	oled_delay_ms(800);
	while(*s) 
	{
		if((*s) >= 128) 
		{
			...
			for (k=0;k<HZnum;k++) 
			{
				oled_setwindows(16*x,16*y,x+16-1,y+16-1);
				for(i=0;i<16*2;i++)
				{
					for(j=0;j<8;j++)
					{
						if(stxt[i]&(0x80>>j)) oled_write_data_16Bit(fc);
						else 
						{
						if (fc!=bc) oled_write_data_16Bit(bc);
						}
					}
				}
			}
...
void oled_main(void)
{
	...
	// char * s = tfont16.Mak
	// 文字填充
	Fast_DrawFont_GBK16(1,1,1,1,stxt);
